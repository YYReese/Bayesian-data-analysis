...
car.data <- dataCar %>% select(veh_value, exposure, 
                               ...
                               area, agecat, clm)
car.data.centred <- car.data %>% 
  mutate(veh_value=veh_value-mean(veh_value), exposure=exposure-mean(exposure),
         veh_age=veh_age-mean(veh_age))
```

Data block
```{r}
n <- nrow(car.data)
body.dum <- dummy(car.data.centred$veh_body)
agecat.dum <- dummy(car.data.centred$agecat)
gen.dum <- dummy(car.data.centred$gender)
area.dum <- dummy(car.data.centred$area)
car.logit.data <- list(n=n, body=body.dum[,-1],
                     agecat=agecat.dum[,-1],
                     gender=gen.dum[,-1],
                     area=area.dum[,-1],
                     veh_value=car.data.centred$veh_value,
                     exposure=car.data.centred$exposure,
                     veh_age=car.data.centred$veh_age)
```
...
car.inla.data <- car.data.centred %>% 
  mutate(veh_body=as.factor(veh_body),area=as.factor(area),agecat=as.factor(agecat))
