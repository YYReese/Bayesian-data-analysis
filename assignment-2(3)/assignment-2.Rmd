...
car.post.pred.test <- function(replicates,car.inla.data){
  ind.clm0 <- which(car.inla.data$numclaims==0)
  clm0 <- replicates[ind.clm0,]
  
  ind.clm1 <- which(car.inla.data$numclaims==1)
  clm1 <- rowMeans(replicates[ind.clm1,])
  
  ind.clm2 <- which(car.inla.data$numclaims==2)
  clm2 <- rowMeans(replicates[ind.clm2,])
  
  ind.clm3 <- which(car.inla.data$numclaims==3)
  clm3 <- rowMeans(replicates[ind.clm3,])
  
  ind.clm4 <- which(car.inla.data$numclaims==4)
  clm4 <- rowMeans(replicates[ind.clm4,])

  par(mfrow=c(2,3))
  par(mar=c(2,2,2,2))
  xmin=min(c(clm0,0))-2
  xmax=max(c(clm0,0))+2
  hist(clm0,col="gray40",main="0 claim",xlim=c(xmin,xmax))
  abline(v=0,col="red",lwd=2)
  
  xmin=min(c(clm1,1))-2
  xmax=max(c(clm1,1))+2
  hist(clm1,col="gray40",main="1 claim",xlim=c(xmin,xmax))
  abline(v=1,col="red",lwd=2)
  
  xmin=min(c(clm2,2))-2
  xmax=max(c(clm2,2))+2
  hist(clm2,col="gray40",main="2 claims",xlim=c(xmin,xmax))
  abline(v=2,col="red",lwd=2)
  
  xmin=min(c(clm3,3))-2
  xmax=max(c(clm3,3))+2
  hist(clm3,col="gray40",main="3 claims",xlim=c(xmin,xmax))
  abline(v=3,col="red",lwd=2)
  
  xmin=min(c(clm4,4))-2
  xmax=max(c(clm4,4))+2
  hist(clm4,col="gray40",main="4 claims",xlim=c(xmin,xmax))
...
barplot(table(yrep.b)/ncol(yrep.b),xlab="y",
        ylab="Pr(y* |y )")
```
