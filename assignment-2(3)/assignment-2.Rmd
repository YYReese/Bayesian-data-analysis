...
```{r}
fittedvalues.I <- results.car.logit.I$summary.fitted.values

fitted.post.pred=rbinom(n=n, size=1, prob=fittedvalues.I)
...
fittedvaluesm=m3.I$summary.fitted.values$mean[nrow(newdata)/2:nrow(newdata)]
plot(car.data.centred$exposure,car.data.centred$clm, col="black", xlab="exposure",
ylab="clm")
legend("topleft", c("1", "2", "3", "4","5","6"),
col = c("blue", "orangered4", "orangered", "orange","yellow","green"),
       lty = c(1,5,6,3,2,4), lwd = 3,bty = "n")

points(rep(1,l1),fittedvaluesm[agecat.dum[,1]], col="blue",lwd=3)
points(rep(2,l2),fittedvaluesm[agecat.dum[,2]], col="orangered4",lwd=3)
points(rep(3,l3),fittedvaluesm[agecat.dum[,3]], col="orangered",lwd=3)
points(rep(4,l4),fittedvaluesm[agecat.dum[,4]], col="orange",lwd=3)
points(rep(5,l5),fittedvaluesm[agecat.dum[,5]], col="yellow",lwd=3)
points(rep(6,l6),fittedvaluesm[agecat.dum[,6]], col="green",lwd=3)


lines(x, fittedvaluesm[(lx+1):(2*lx)], col="orangered4", lwd=3, lty=5)
lines(x, fittedvaluesm[(2*n+1):(3*n)], col="orangered", lwd=3, lty=6)
lines(x, fittedvaluesm[(3*n+1):(4*n)], col="orange", lwd=3, lty=3)
```
...
For the choice of priors, we follow the common choice of normal distribution in general. Since we believe the typical probability of a claim is between $0.05 \sim 0.95$, then $\log(1/19)<\log(\frac{p}{1-p})<\log(19)$. Equivalently, we would like the intercept $\beta_0$ to mostly take values from $\log(1/19)$ to $\log(19)$. Therefore, the prior for $\beta_0$ is set to be $\mathcal{N}\left(0,\frac{\log(1/19)}{3}\right)$.
