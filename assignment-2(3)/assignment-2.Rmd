...
results.car.poi.rd <- inla(formula = numclaims ~ veh_value+exposure+
               veh_age+gender*agecat+f(area, model="iid")+f(veh_body, model="iid"), 
...
```{r}
#Priors for the regression coefficients.
prior.beta.a <- list(mean.intercept = 0, prec.intercept =  0.01,
                       mean =0, prec = 0.1)

prec.prior <- list(prec=list(prior = "loggamma", param = c(0.1, 0.1)))
#Fitting the model in INLA
#"control.fixed=prior.beta" sets regression coeff. priors.
results.study.a <- inla(formula = stroop_test_performance ~ gender+on_a_diet+
