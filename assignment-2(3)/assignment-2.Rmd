...
results.car.logit$summary.fixed$mean[2] * sd(dataCar$veh_value)+mean(dataCar$veh_value)

cat("Regression coefficient for exposure after transformation to the original scale: \n")
results.car.logit$summary.fixed$mean[3] * sd(dataCar$exposure)+mean(dataCar$exposure)
```
...
x <- exp( beta0+beta1+beta2+beta3+beta4+beta5+beta6+beta7)
...
For the non-categorical variable, we need to transfer the coefficients back to the original scale to better interpret the data. The transformed values are shown below. For the vehicle value, we expect $6.12$ times increase of the expected number of claims in every unit increase on the veh_value. Similarly, we expect $1.86$ times increase of the expected number of claims in every unit increase on the exposure. 

```{r}
cat("Regression coefficient for veh_value after transformation to the original scale: \n")
results.car.log$summary.fixed$mean[2] * sd(dataCar$veh_value)+mean(dataCar$veh_value)
cat("Take the exponential:\n")
exp(results.car.log$summary.fixed$mean[2] * sd(dataCar$veh_value)+mean(dataCar$veh_value))

cat("Regression coefficient for exposure after transformation to the original scale: \n")
results.car.log$summary.fixed$mean[3] * sd(dataCar$exposure)+mean(dataCar$exposure)
cat("Take the exponential:\n")
exp(results.car.log$summary.fixed$mean[3] * sd(dataCar$exposure)+mean(dataCar$exposure))
```
