...
model2_string <- "model {
  ...
  for(i in 2:(n+1)) {
    eta[i] ~ dnorm(0,1)
    h[i] <- mu+phi*(h[i-1]-mu)+sigma*eta[i]
  }
  
  
  y.mu[1] <- 0.5
  y.tau[1] <- exp(-h[1])
  y[1] ~ dt(y.mu[1], y.tau[1], k) T(0,10)
  for(i in 2:n) {
    y.mu[i] <- rho*eta[i]*exp(h[i]/2) + beta0 + beta1*y[i-1]
    y.tau[i] <- exp(-h[i])
    y[i] ~ dt(y.mu[i], y.tau[i],k)T(0,10)
  }
...
num.chains <- 5
# Compile and run the model
model1 <- jags.model(file=textConnection(model2_string), 
                        data=eur2usd.data2,
                        n.chains=num.chains)
...
**Discuss the results.** ##\# Posterior predictive checks for (a)
...
num.chains <- 5
# Compile and run the model
model1_check <- jags.model(file=textConnection(model_string), 
                        data=eur2usd.data,
                        n.chains=num.chains)
# Burnin for 20000 samples
...
```{r}
# Running the model, monitoring the variable theta
res.model1_check=coda.samples(model1_check,variable.names=c("y_rep"), n.iter=60000,progress.bar="none")
